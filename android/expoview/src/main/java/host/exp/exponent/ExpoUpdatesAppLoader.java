  ...
  private void setShouldShowAppLoaderStatus(RawManifest manifest) {
    ...
    try {
      ...
      if (mShouldShowAppLoaderStatus) {
        ...
        if (sdkVersion != null &&
            ABIVersion.toNumber("39.0.0") > ABIVersion.toNumber(sdkVersion) &&
            "snack".equals(manifest.getSlug()) &&
            manifest.getBundleURL().startsWith("https://d1wp6m56sqw74a.cloudfront.net/%40exponent%2Fsnack")
        ) {
          mShouldShowAppLoaderStatus = false;
        }
      }
